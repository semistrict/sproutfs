...
// TestStoppingAVMPublishesWhatItHeldAndGivesTheFramesBack: a stop is the
...
// frames go back to the pager and the handle is released, so another host can
// open the VM at exactly the bytes the stop published.
func TestStoppingAVMPublishesWhatItHeldAndGivesTheFramesBack(t *testing.T) {
	h := newHostHarness(t)
	...
	// Written after the last checkpoint, so only this host's frames hold it:
	...
	// The checkpoint it reports is the instant the VM comes back at, and
...
// TestStoppingAVMAForkInstantHoldsIsRefused: a child reads the pages no
// checkpoint holds out of the frames the parent's seal froze, and those frames
// are the parent's VMM process's. Closing that process while a child is still
// pulling them takes the instant out from under it, so a stop of a sealed VM is
// refused exactly as a delete of one is — and the parent goes on running.
func TestStoppingAVMAForkInstantHoldsIsRefused(t *testing.T) {
	...
	if _, err := h.hosts[0].Stop(t.Context(), "parent"); !errors.Is(err, volume.ErrSealed) {
		t.Fatalf("stopping a VM a fork instant holds = %v, want ErrSealed", err)
	}
	...
	if serving := h.hosts[0].Status().Serving; len(serving) != 1 || serving[0] != "child" {
		t.Fatalf("the refused stop gave up the instant the child reads: %v", serving)
	}
