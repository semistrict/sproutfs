...
func TestAMigrationStoppedPartWayDiscardsTheVM(t *testing.T) {
	...
	vm, err := h.hosts[0].Volumes().Create(t.Context(), "vm-1", twoRegionVolumes)
	if err != nil {
		t.Fatal(err)
	}
	...
	// The regions give their volumes up in name order, so a seal held on the
	// second is a handoff that fails after the first has already happened.
	if err := source.regions["root"].Seal(t.Context()); err != nil {
		t.Fatal(err)
	}

	_, err = h.hosts[0].Migrate(t.Context(), "vm-1", h.pages[1])
	if !errors.Is(err, vmmigrate.ErrStopped) {
		t.Fatalf("a migration that gave up one region and failed on the next = %v", err)
	}
