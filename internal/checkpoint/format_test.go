...
func loadFixtureObjects(t *testing.T, store *Store, dir string) {
	...
	if count == 0 {
		t.Fatalf("the fixture at %s holds no objects; run go test ./internal/checkpoint -update", dir)
	}
}
...
func readFixture(t *testing.T, path string) []byte {
	...
	if err != nil {
		t.Fatalf("%v; run go test ./internal/checkpoint -update to write the fixtures", err)
	}
	return data
}
