...
// every store it refuses is refused for the age of what the region holds, not
// for the room it has.
...
func TestAStorePastTheLossWindowWaitsForItsCheckpoint(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		...
		case got := <-requested:
			if got != r {
				t.Fatalf("the pager asked to checkpoint %v, want the region over its window", got)
			}
		default:
...
func TestADisabledLossWindowNeverWaits(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		f := windowFixture(t, 0)
		r, m, _ := f.region(4)
		f.h.SetPressure(vmmemory.Pressure{Checkpoint: func(*vmmemory.Region) bool {
...
// budget stall ends: the region's owner is told, and it stops that VM
...
func TestAStorePastTheLossWindowWithNoCheckpointComingIsAStall(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		...
		if stoppedRegion != r || !errors.Is(stoppedCause, vmmemory.ErrWindowStalled) {
			t.Fatalf("the pager stopped %v for %v, want the region over its window", stoppedRegion, stoppedCause)
		}
...
// The window is a VM's, because the checkpoint is: a region holding nothing old
// of its own still waits while another region of the same VM holds a write past
// the window, since the checkpoint that ends the wait covers both. The pager
// knows nothing about VMs — it asks whoever owns the region.
func TestTheLossWindowIsTheVMsRatherThanOneRegions(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		f := windowFixture(t, lossWindow)
		ram, ramMapping, ramBacking := f.region(4)
		disk, diskMapping, _ := f.region(4)
		oldest := func(region *vmmemory.Region) time.Time {
			// Both regions belong to one VM, so each answers for the pair.
		...
		time.Sleep(lossWindow + time.Second)
		// This region holds nothing at all, so only its VM's other region can be
		...
		select {
		case err := <-stored:
			t.Fatalf("a store in a region whose VM is past the window did not wait: %v", err)
		default:
		}
		f.mustCheckpoint(ram, ramBacking)
		if err := <-stored; err != nil {
			t.Fatalf("the store failed after its VM's other region was checkpointed: %v", err)
		}
		access(t, disk, diskMapping, 0, true)[0] = 21
	})
}

// A handoff carries how old the region's oldest unpublished write is, so the
...
// A region with nothing unpublished hands off an age of zero: there is nothing
// for the destination to date, and a destination given one would start its
// guest already inside a window it never wrote in.
func TestAHandoffOfACleanRegionCarriesNoAge(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		f := windowFixture(t, lossWindow)
		r, m, b := f.region(4)
		access(t, r, m, 0, true)[0] = 11
		...
		if age != 0 {
			t.Fatalf("a region holding nothing unpublished hands off an age of %s, want none", age)
		}
	})
}
...
func TestAReceivedRegionInheritsItsSourcesWindow(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		f := windowFixture(t, lossWindow)
		r, m, b := f.region(4)
		f.h.SetPressure(vmmemory.Pressure{Checkpoint: func(*vmmemory.Region) bool { return true }})
