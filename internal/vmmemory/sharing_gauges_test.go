...
func wantRegion(t *testing.T, r *vmmemory.Region, resident, private, shared int, what string) {
	t.Helper()
...
// Two regions that inherited the same checkpoint map one resident page each,
...
func TestSharingGaugesCountEveryAliasOfAResidentPage(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		...
		wantSharing(t, sharing(t, f).Ram, 4, 8, "two regions of four shared pages")
		wantRegion(t, a, 4, 0, 4, "the first region")
		wantRegion(t, b, 4, 0, 4, "the second region")
	})
}

// A store is where a shared page stops being shared: the writer takes a private
// page of its own, the arena holds one page more, and neither region counts that
...
func TestAStoreTakesOnePageOutOfTheSharedSet(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		...
		wantRegion(t, a, 4, 1, 3, "the region that stored")
		wantRegion(t, b, 4, 0, 3, "the region that did not")
	})
}

// The gauges are split by what the region is to its guest, because RAM and PMEM
...
func TestSharingGaugesReportRAMAndPMEMSeparately(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		...
		stats := sharing(t, f)
		wantSharing(t, stats.Ram, 2, 2, "the RAM region alone")
		wantSharing(t, stats.Pmem, 3, 3, "the PMEM region alone")
	})
}

// Eviction is the other direction: the page leaves the arena and every region
...
func TestEvictingASharedPageLowersEveryGauge(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		...
		// The arena is full, so the next region's fault takes the page both of
		// them touched first.
		...
		wantRegion(t, a, 3, 0, 3, "the first region")
		wantRegion(t, b, 3, 0, 3, "the second region")
		wantRegion(t, c, 1, 0, 0, "the region whose fault evicted it")
	})
}
