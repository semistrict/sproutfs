...
func TestManagedPagerReadinessMeasurements(t *testing.T) {
	...
	for page := 0; page < pages; page += 2 {
		start := time.Now()
		a.request(fmt.Sprintf("kvmread 0 %d", page*hugePageSize), fmt.Sprintf("kvm %d", byte(page+1)))
	...
	for page := 0; page < pages; page += 2 {
		b.request(fmt.Sprintf("kvmread 0 %d", page*hugePageSize), fmt.Sprintf("kvm %d", byte(page+1)))
	}
