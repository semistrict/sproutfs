...
// TestAFrameASiblingWillNotGiveUpDoesNotFailTheVMEvictingIt. Children of one
// fork share every frame they inherited, which is the whole point of forking
// them onto one host. A machine whose memory session has died can no longer
// take a mapping away, so a frame it holds can never be reused — but the
...
// frame with it. The eviction takes another victim instead; the frame stays
// where it is until the region that holds it is closed.
func TestAFrameASiblingWillNotGiveUpDoesNotFailTheVMEvictingIt(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		f := newConfiguredFixture(t, vmmemory.Config{ResidentPages: 2, LogicalPages: 32,
			DirtyPages: 2, ReadAheadPages: 1})
		a, am, _ := f.region(8)
		b, bm, _ := f.region(8)
		// One frame, two children of one lineage reachable from it.
		access(t, a, am, 0, false)
		access(t, b, bm, 0, false)
		if am.pages[0].slot != bm.pages[0].slot {
			t.Fatal("the two regions did not share the frame of one stored identity")
		}
		// The second machine stops answering. Its region has taken no failure of
		...
		// The arena holds two frames and this region walks eight pages, so every
		// fault past the second evicts, and the least recently used frame is the
		// one the dead machine will not give up.
		for page := uint64(1); page < 8; page++ {
			if err := a.Fault(t.Context(), page, false); err != nil {
				t.Fatalf("a fault of a live VM whose sibling will not give up a shared frame: %v", err)
			}
		}
		...
		if err := b.Fault(t.Context(), 1, false); err == nil {
			t.Fatal("the region whose mapping stopped answering went on serving faults")
		}
	})
}
