...
// Flush is a guest's flush of this region reaching the pager: what a session's
...
// disk's, so a flush of a RAM region is answered with an error at once.
func (r *Region) Flush(done func(error)) {
	if r.kind != Pmem {
		done(fmt.Errorf("vmmemory: a flush of a %s region", r.kind))
		return
	}
	r.host.countFlush()
	r.deliverFlush(done)
...
func (r *Region) deliverFlush(done func(error)) {
