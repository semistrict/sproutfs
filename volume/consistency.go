...
func CheckDeployment(ctx context.Context, store platform.ObjectStore, prefix platform.ObjectPrefix, allow ...Allowance) error {
	...
	audit := &audit{store: store, records: records, checkpoints: checkpoints,
		prefix: prefix, base: deploymentBase(prefix),
...
func (a *audit) run(ctx context.Context) error {
	...
	return a.checkBill(ctx, stored)
}

// list reads the whole namespace, in ascending key order.
func (a *audit) list(ctx context.Context) ([]platform.ObjectMetadata, error) {
	prefix, err := platform.NewObjectPrefix(a.base)
	if err != nil {
		return nil, err
	}
	var listed []platform.ObjectMetadata
	err = platform.ListAll(ctx, a.store, prefix, func(object platform.ObjectMetadata) error {
	...
	slices.SortFunc(listed, func(x, y platform.ObjectMetadata) int {
...
func (a *audit) classify(listed []platform.ObjectMetadata) (identities []string, checkpoints []object, stored map[string]uint64) {
	...
	for _, entry := range listed {
		...
		stored[found.vm] += uint64(entry.Size)
		if found.record {
			identities = append(identities, found.vm)
			continue
		}
		sequence, index, ok := checkpointKey(found.below)
		if !ok {
...
// every tenant the listing holds anything of, the VMs of no tenant included, is
// exactly what the listing holds under each VM. classify has required every key
// to be under some VM, so the bill summed over VMs is then every byte the store
// holds, and no byte is billed twice. A tenant whose report fails on a key
// under no VM is not compared, because classify has reported that key.
func (a *audit) checkBill(ctx context.Context, stored map[string]uint64) error {
	tenants := map[string]bool{"": true}
	for vm := range stored {
		tenants[control.TenantOf(vm)] = true
	}
	billed := make(map[string]uint64, len(stored))
	unbilled := make(map[string]bool)
	for _, tenant := range slices.Sorted(maps.Keys(tenants)) {
		report, err := StoredBytes(ctx, a.store, a.prefix, tenant)
		if errors.Is(err, ErrCorrupt) {
			unbilled[tenant] = true
			continue
		}
		if err != nil {
			return err
		}
		for vm, bytes := range report {
			billed[vm] = bytes
		}
	}
	for _, vm := range slices.Sorted(maps.Keys(billed)) {
		if _, held := stored[vm]; !held {
			a.report("", 0, fmt.Errorf("%s is billed %d bytes and the store holds nothing under it", vm, billed[vm]))
		}
	}
	for _, vm := range slices.Sorted(maps.Keys(stored)) {
		if unbilled[control.TenantOf(vm)] || billed[vm] == stored[vm] {
			continue
		}
		a.report("", 0, fmt.Errorf("%s is billed %d bytes and the store holds %d under it", vm, billed[vm], stored[vm]))
	}
	return nil
}
