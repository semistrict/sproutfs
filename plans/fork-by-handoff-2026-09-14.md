...
sealed frames through the pager. Every fork therefore publishes a checkpoint
...
A checkpoint has two halves: the instant (pause, save VMM state, seal the
dirty set, resume) and the upload. A fork, like a migration, needs only the
instant. The parent is treated as a migration source that keeps running:
...
2. Build a handoff for the child as `vmmigrate` builds one for a migration:
   ...
   frames stay the parent's and the child reads them by lineage identity.
...
4. Start the child. On the parent's host the child shares the sealed frames
...
The parent's sealed frames are released to it page by page as it writes
...
- The pin is on the last published sequence, which may be older than the
  fork instant; nothing between it and the instant is in the store until one
...
shares frames and the cross-host case is one flag: `sproutfsctl fork
...
Sim tests: same-host fork shares frames and publishes nothing on the parent;
