...
| Count shared frames once; admit before allocating | `takeFree` reserves a run before arena allocation; aliases reference slots. `TestSharedFrameIsChargedOnceUntilLastAliasDetaches`, physical-cleanup failure tests and the 2 MiB cache-first fault test pass. |
...
| Required reads work when frames fill RAM | Cache misses use an unretained transient copy from I/O headroom when no retained reservation fits. The full-frame read regression checks data and absence of retained cache. |
