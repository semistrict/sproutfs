...
- **Fixed: the probe build's `TestSealTakingAReclaimingPagesReservationKeepsItsBytes` panicked under load, and a refault acting on a decision a checkpoint had already overtaken was why.** The pager's audit reported `probe bind: page N of region … was given slot -1 from outside its own store path while it owned generation G, and now takes slot S — a lost write`, from the store's own `takePrivate`, in about one lane in eight under contention.

  No write was lost. The page the guest was bound to held the bytes the guest last stored at every step: with the finding made non-fatal, thirty lanes ran to the end and the test's own `reads %d, want the %d the guest stored` never fired once. What the audit had caught was the pager granting a binding the right to store into memory after that binding's dirty epoch had already ended.

  A reclaim for a private page gives the region up while it looks for an arena slot, so a seal and a retire can both run inside a fault that has already decided what the page it is serving is. The store path re-decides across its own reclaim — `fault` compares the checkpoint's copy before and after — and the spill refault in `loadOnce` did not. A checkpoint taken in that window retires the page: the volume holds its bytes, the reservation that spilled them goes back, and the binding is clean. The refault then bound a private page into it anyway. That page owns neither a reservation nor a checkpoint, which is exactly the state `evictBatch` punches without writing it anywhere, and it is named by nothing, so nothing that inherits the identity the checkpoint gave it can map it: every other region of that volume reads its own copy of bytes this host is already holding. The audit's generation bookkeeping is right as it stands — the binding it was owed something newer by was one the pager should never have granted.

  `loadOnce` now reads the page's dirty state and the checkpoint's copy of it together, before and after the reclaim, and decides what the page is again from the top when either moved (`internal/vmmemory/fault.go`, `bindings.go`, `privateEpoch`). `TestARefaultWhoseCheckpointRetiresWhileItReclaimsGivesThePageToTheVolume` drives the interleaving through a reclaim seam and is red every run without the fix, in both builds: the ordinary build on the second region reading its own copy, the probe build on that same panic with that same stack. Measured on 2026-09-22 on a fifteen-core machine, 50 lanes each with a detector on the grant itself: **10 of 50 lanes before, 0 of 50 after**, which at that rate leaves about a 1-in-70,000 chance of a clean reading by luck. The panic that the lanes actually produce is rarer than the grant that causes it — about 1 lane in 50 on this machine against 1 in 8 on the eight-core one the earlier counts came from — so it is 0 of 150 lanes after the fix, and it is the grant's count that carries the result.
