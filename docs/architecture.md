...
| [Checkpoint](volumes.md) | Its data and one index object under `vm/<id>/ckpt/<seq>/`, and nothing else. The data is `part/<n>`: a run of members — the VMM state, when one was saved, then the dirty pages in page order, then compaction's rescues — filled to 64 MiB and closed by a table of at most 1 MiB and a fixed 32-byte trailer, so a part describes itself. Publishing in page order is what lets a read of a range of a volume fetch the members of consecutive pages as one **extent**, one ranged read, so a pager's cold 2 MiB read-ahead run of 512 4 KiB pages is two requests — the segment that locates them and the extent their members lie in — rather than one per page. Written last is `index`: a fixed record, the page-table segments this checkpoint changed, the **root**, and the record again, whose create-if-absent PUT is the publication's commit. The root records each volume's geometry — its page size and the pages one segment covers, 512 MiB of a 2 MiB-page volume and 64 MiB of a 4 KiB-page one — and addresses each segment of that volume's page table inside the index object of the checkpoint that wrote it, keeping an earlier checkpoint's address for every segment this one did not change, and lists every checkpoint it reads a page from or addresses a segment in, plus the ones its own compaction emptied and spares for a checkpoint. It names no parent: a root is complete on its own, one GET of the end of the index object yields it, and a reader fetches the segments a range falls in. |
| Overlay | The in-memory writes of one open VM since its selected checkpoint, published by the next one. It survives nothing. |
| [Pager](vm-memory.md) | The host's own page cache for guest memory: a shared memfd arena serving attached regions, keyed by page identity, in place of the kernel's page cache and swap — [why](vm-memory.md#why-a-pager-of-its-own). A host runs one instance per kind of region, one for RAM and one for PMEM, each with its own arena, its own spill file and its own page, and a region attaches to the pager of its kind. A pager's page is the same for every region it serves, so it refuses a volume published in any other page size when that volume is attached. RAM runs 4 KiB over an arena of ordinary memory and PMEM runs 2 MiB over one of the node's HugeTLB pool, on a real host and in the simulation alike: a guest's store into its memory copies and owns 4 KiB, while inherited RAM is still mapped in contiguous runs of up to 2 MiB with one command each. The mapping protocol carries each session's page and its arena's kind, and both ends refuse a mismatch before any guest memory exists. |
| [Host](hosting.md) | `internal/host`: everything one host does. Its `Host` opens VMs over object storage and the cluster network, checkpoints them on an interval, fences a VM a later writer took, and serves migration and fork pages; the supervisor around it owns the two pagers and the VMM processes behind the host API, imports guest images into the templates VMs are forked from, and reaches the agent in a guest. `cmd/sproutfs-host` is its configuration, its HTTP handlers and the adapters it chooses. |
