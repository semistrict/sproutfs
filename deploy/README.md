...
| POST | `/vms` | `{"template","memory"?,"disk"?,"vcpus"?}`: allocate a ULID and create the VM at that shape on the host whose guests have promised the least of its arena, refusing with 503 when the VM's RAM — what it asks for, or else its template's — fits on none |
